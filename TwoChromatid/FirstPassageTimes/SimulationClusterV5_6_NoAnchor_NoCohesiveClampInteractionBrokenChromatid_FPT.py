...
from parameters import n_process_cores, N_strand, N1, M, LIFETIME, SEPARATION, SEPARATION_AFTER_DSB, trajectoryLength, simulation_cycles, steady_state_relax_1D, dsb_time, dsb_present, delay_before_fpt, ctcf_sites, cohesion_sites, Stop_Walkoff, LIFETIME_CTCF, dsb_site, dsb_ends_connected, cohesion_recruitment, kbp_offset_dsb_homology, homology_binding, homology_site, p_bond, distance_threshold, B_cohesion, B_ctcf, contacts_cutoff, use_random_tads, num_tads_per_strand, min_spacing
...
def add_dsb_template_bonds(current_bonds, positions, cohesions, occupied, occupied_cohesive, dsb_location=dsb_site,
    ...
    for strand_pair in range(M):
        ...
        for side in ["left", "right"]:
            ...
            dsb_index = strand_pair * N2 + dsb_location + offset
            if occupied_cohesive[dsb_index] != 0: # Only one cohesin or cohesive cohesin at a single site
                continue
...
class bondUpdater(object):
    ...
    def setup(self, bondForce,  candidate_LEF_loading_sites, use_candidate_LEF_sites=False, blocks=100, smcStepsPerBlock=1, N1=N1, N2=N1*2, M=M, kbp_offset=0, n_process=1, iteration=None):
        ...
        if cohesion_recruitment and dsb_present and iteration > dsbIteration:
            for strand_pair in range(M):
                for offset in [-kbp_offset - 1, kbp_offset]:
                    dsb_index = strand_pair * N2 + dsb_site + offset
                    for i in range(N1):
                        hom_index = strand_pair * N2 + N1 + i
                        bond = (min(dsb_index, hom_index), max(dsb_index, hom_index))
    ...
    def step(self, context, homology_binding=False, positions=None, args=None, iteration=None, distance_threshold=5, p_bond=1, kbp_offset=0, verbose=False):
        """
        ...
        if homology_binding and dsb_present and iteration>dsbIteration:
            new_homology_bonds = add_dsb_homology_bonds(
                current_bonds=self.curBonds, 
                positions=positions, 
                distance_threshold=distance_threshold, 
                p_bond=p_bond,
                kbp_offset=kbp_offset
                )
        ...
        return self.curBonds, pastBonds, args
...
for cycle in range(simulation_cycles):
    ...
    LEFpositions = []
    occupied = np.zeros(N)
    occupied_cohesive = np.zeros(N) # in order to prevent LEFs from interacting with cohesive cohesin on the broken chromatid, cohesive cohesin occupancy is tracked using this separate array so it doesn't flag the site in the occupied array
    occupied[0] = 1
    occupied[-1] = 1 
    cohesins = []
    cohesins_over_time = []
    cohesions = []
    cohesions_over_time = []
    LEFDeficit = 0 # to catch cases where no LEFs could be loaded in order to recover LEF density on next iteration
    ...
    if N_strand == "two":
        fpt_reached = {
            'left': np.full((M, N1*2), False),
            'right': np.full((M, N1*2), False) 
        }
        first_pass_time = {
            'left': np.full((M, N1*2), np.inf),
            'right': np.full((M, N1*2), np.inf)
        }
    ...
    for iteration in range(simInitsTotal):
        ...
        if iteration == dsbIteration:
            ...
            if dsb_present:
                ...
                if False: # No BE properties in this case (ONLY WORKS IF ENDS ARE TETHERED!!!!!!!!!!)
                    if N_strand == "two":
        ...
        milker.setup(bondForce=a.force_dict['harmonic_bonds'],
                    ...
                    M=M,
                    iteration=iteration,
                    kbp_offset=kbp_offset_dsb_homology,
        ...
        for i in range(restartSimulationEveryBlocks):        
            ...
            if cohesion_recruitment and dsb_present and iteration > dsbIteration:
                cohesions = add_dsb_template_bonds(current_bonds=milker.curBonds, positions=a.get_data(), cohesions=cohesions, occupied=occupied, occupied_cohesive=occupied_cohesive, distance_threshold=distance_threshold, p_bond=p_bond, kbp_offset=kbp_offset_dsb_homology)
            milker.LEFpositions = [(cohesin.left.pos, cohesin.right.pos) for cohesin in cohesins]
            milker.LEFpositions.extend((cohesion.damaged.pos, cohesion.template.pos) for cohesion in cohesions)

            curBonds, pastBonds, args = milker.step(a.context, homology_binding=homology_binding, positions=a.get_data(), args=args, iteration=iteration, distance_threshold=distance_threshold, p_bond=1, kbp_offset=kbp_offset_dsb_homology, verbose=False) 
            milker.LEFpositions = [(cohesin.left.pos, cohesin.right.pos) for cohesin in cohesins] # Probably not necessary because no homologies here, but I added it just to be safe
            milker.LEFpositions.extend((cohesion.damaged.pos, cohesion.template.pos) for cohesion in cohesions)

            cohesins_over_time.append([(cohesin.left.pos, cohesin.right.pos) for cohesin in cohesins])
            cohesions_over_time.append([(cohesion.damaged.pos, cohesion.template.pos) for cohesion in cohesions])

            if i % saveEveryBlocks == (saveEveryBlocks - 1):  
                a.do_block(steps=steps, save=True) # saving of trajectory files can be disabled here to avoid using too much storage
            else:
                a.integrator.step(steps)  # do steps without getting the positions from the GPU (faster)

            if N_strand == "two":
                if i + iteration * restartSimulationEveryBlocks >= dsb_time + delay_before_fpt: # the delay_before_FPT offset is intended to represent the time delay between the DSB and recruitment of repair proteins
                    pos = a.get_data()
                    for j in range(M): 
                        offset = j * N1 * 2
        ...
        # Early exit if all FPTs are reached
        if N_strand == "two":
            if np.all(fpt_reached['left']) and np.all(fpt_reached['right']):
                print("All FPTs reached — saving outputs and ending simulation early.")

                # This save code is redundant against the code below, however it exists for safety to guarantee that all necessary files are saved.
    ...
    # Save LEF and cohesive cohesin (recruited) positions
    np.save(
        f"{save_folder_name}/LEF_Positions.npy",
        np.array(cohesins_over_time, dtype=object)
    )

    np.save(
        f"{save_folder_name}/RecruitedCohesiveCohesin_Positions.npy",
        np.array(cohesions_over_time, dtype=object)
    )
