...
for cycle in range(simulation_cycles): # note that this won't regenerate new TADs if this is repeated for each cycle as currently written. I have been using just 1 cycle and running many GPUs.
    save_folder_name = f"trajectory{cycle}"
    if not os.path.exists(save_folder_name):
        os.mkdir(save_folder_name)

    # This is new between 5.6 and 5.7. This saves the TAD boundary positions for downstream analysis
    np.save(
        f"{save_folder_name}/TAD_Boundary_Positions.npy",
        np.array(pair_cohesion_sites, dtype=object)
    )
    ...
    for iteration in range(simInitsTotal):
        ...
        for i in range(restartSimulationEveryBlocks):        
            ...
            if cohesion_recruitment and dsb_present and iteration > dsbIteration:
                cohesions = add_dsb_template_bonds(current_bonds=milker.curBonds, positions=a.get_data(), cohesions=cohesions, occupied=occupied, distance_threshold=distance_threshold, p_bond=p_bond, kbp_offset=kbp_offset_dsb_homology)
            milker.LEFpositions = [(cohesin.left.pos, cohesin.right.pos) for cohesin in cohesins]
            milker.LEFpositions.extend((cohesion.damaged.pos, cohesion.template.pos) for cohesion in cohesions)
